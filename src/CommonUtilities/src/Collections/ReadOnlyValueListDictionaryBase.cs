...
/// <summary>
/// Represents an abstract base class for a read-only, generic dictionary that maps keys to a list of values.
/// </summary>
    ...
    where TKey : notnull
{
    ...
    /// <inheritdoc />
    public IReadOnlyList<TValue> this[TKey key] => _dictionary[key];
    ...
    /// <inheritdoc />
    public IReadOnlyList<TValue> GetValues(TKey key) => _dictionary.GetValues(key);
    ...
    /// <inheritdoc />
    public bool TryGetValues(TKey key, out IReadOnlyList<TValue> values) => _dictionary.TryGetValues(key, out values);

    /// <inheritdoc />
    public IEnumerator<KeyValuePair<TKey, IReadOnlyList<TValue>>> GetEnumerator() => _dictionary.GetEnumerator();
