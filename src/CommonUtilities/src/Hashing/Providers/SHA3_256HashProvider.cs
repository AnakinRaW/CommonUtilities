...
internal class SHA3_256HashProvider : HashAlgorithmProviderBase
{
    public override HashTypeKey SupportedHashType => HashTypeKey.SHA3_256;

    protected override int HashDataNetCore(ReadOnlySpan<byte> source, Span<byte> destination)
    {
        return SHA3_256.HashData(source, destination);
    }
    ...
    {
        return SHA3_256.HashDataAsync(source, destination, cancellation);
    }
