...
public abstract class CollectionsTestSuite
{
    ...
    public static IEnumerable<object[]> GetEnumerableTestData()
    {
        foreach (var collectionSizeArray in ValidCollectionSizes())
        {
            ...
            if (count >= 1)
            {
                yield return [count, count, 0, 0];               // Enumerable of the same size
                yield return [count, count - 1, 0, 0];           // Enumerable that is 1 smaller
                yield return [count, count, 1, 0];               // Enumerable of the same size with 1 matching element
                yield return [count, count + 1, 1, 0];           // Enumerable that is 1 longer with 1 matching element
                yield return [count, count, count, 0];           // Enumerable with all elements matching
                yield return [count, count + 1, count, 0];       // Enumerable with all elements matching plus one extra
            }

            if (count >= 2)
            {
                yield return [count, count - 1, 1, 0];           // Enumerable that is 1 smaller with 1 matching element
                yield return [count, count + 2, 2, 0];           // Enumerable that is 2 longer with 2 matching element
                yield return [count, count - 1, count - 1, 0];   // Enumerable with all elements matching minus one
                yield return [count, count, 2, 0];               // Enumerable of the same size with 2 matching element
                yield return [count, count, 0, 1];               // Enumerable with 1 element duplicated
            }

            if (count >= 3)
            {
                yield return [count, count, 0, 1];               // Enumerable with all elements duplicated
                yield return [count, count - 1, 2, 0];           // Enumerable that is 1 smaller with 2 matching elements
            }
        }
    }
}
